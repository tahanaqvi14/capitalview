...
export default async function StockPage({ searchParams }) {
    ...
    const query = params?.query || ''; // access the query property here
  
  return (
    <main className="bg-gray-50 min-h-screen">
      <header className="flex items-center justify-between px-8 py-4 bg-white border-b">
        <div className="flex items-center gap-2 text-xl font-semibold">
          <div className="w-8 h-8 bg-blue-600 text-white rounded flex items-center justify-center">S</div>
          StockExplorer
        </div>
        ...
        <div className="w-full mt-3 py-4 sm:py-6">
          <div className="mx-auto px-3 sm:px-4">
            <h1 className="text-lg sm:text-xl md:text-2xl font-semibold text-gray-900">
              Search Results for <span className="text-[#193CB8]">"{query}"</span>
            </h1>
